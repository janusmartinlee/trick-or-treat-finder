...
void main() {
  group('Unit Tests: User Preferences Logic', () {
    ...
    test('Given default preferences, When getting preferences, Then returns system theme and EN locale', () async {
    ...
    test('Given system theme, When updating to dark theme, Then theme mode changes to dark', () async {
      ...
      expect(updatedPreferences.locale, equals(const Locale('en', 'US'))); // Locale unchanged
    });

    test('Given EN locale, When updating to ES locale, Then locale changes to Spanish', () async {
      ...
      expect(updatedPreferences.themeMode, equals(ThemeMode.system)); // Theme unchanged
    });

    test('Given preferences changes, When listening to stream, Then stream emits updates', () async {
      // Given - listening to preferences stream
      final streamUpdates = <UserPreferences>[];
      final subscription = useCase.preferencesStream.listen(streamUpdates.add);
    ...
    test('Given multiple preference changes, When applied in sequence, Then all changes persist', () async {
      // Given - default preferences
